...
contract StandardTokenVesting is Ownable {
  ...
  /** @notice Transfers vested tokens to beneficiary.
    ...
    */
  function release(FRSPToken token) public returns (bool){
  ...
  /** @notice Allows the owner to revoke the vesting. Tokens already vested
    ...
    * @param token ERC20 token which is being vested
    */
  function revoke(FRSPToken token) public onlyOwner returns(bool) {
    require(revocable);
  ...
  /** @dev Calculates the amount that has already vested but hasn't been released yet.
    * @param token ERC20 token which is being vested
    */
  function releasableAmount(FRSPToken token) public view returns (uint256) {
